...
class Agent:
    ...
    def getAction(self, state):
        '''
        The `Agent` will receive a `GameState` (from either `pacman.py`,
        `capture.py` or `sonar.py`) and must return an action from
...
class Configuration:
    '''
    A Configuration holds the (x,y) coordinate of a character, along with
    its traveling direction.

    The convention for positions, like a graph, is that `(0, 0)` is the
    lower left corner, `x` increases horizontally and `y` increases
    vertically.  Therefore, north is the direction of increasing `y`, or
    `(0, 1)`.
    '''
...
class Grid:
    ...
    accessed via `grid[x][y]` where `(x, y)` are positions on a Pacman map
    with `x` horizontal, `y` vertical and the origin `(0, 0)` in the bottom
    left corner.
...
class GameStateData:
    ...
    def __init__(self, prevState = None):
        '''
        Generate a new data packet by copying information from its
        predecessor.
        '''
        if prevState != None:
